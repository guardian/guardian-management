...
class MockHttpServletRequest(val url: String = null, var contextPath: String = "") extends HttpServletRequest {
    ...
    if (url.toLowerCase.startsWith("http")) {
      ...
      computeRealPath(url).split('?') match {
        case Array(path, query) =>
          this.path = path; queryString = query
        case Array(path) =>
          this.path = path; queryString = null
  ...
  def addBasicAuth(user: String, pass: String) {
    val hashedCredentials =
      base64Encode((user + ":" + pass).getBytes)
  ...
  def getDateHeader(h: String): Long = 0

  def getIntHeader(h: String): Int = 0
